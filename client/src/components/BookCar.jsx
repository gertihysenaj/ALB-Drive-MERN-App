...
function BookCar() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:8000/api/cars/64a5a22827e070bf5a464213`,{ withCredentials: true } )
      .then((res) => {
        setSelectedCar(res.data.data);
        setCarType(res.data.data.mark + " " + res.data.data.model); // Set the car type based on fetched car data
      })
  ...
  // booking car
  const [carType, setCarType] = useState(carID);
...
////////////////////////////////////////////////////////////////////////////////////////
  ...
  const [paymentData, setPaymentData] = useState(null);  // Declare paymentData state variable
  ...
  const confirmBooking = async (e) => {
    ...
    // Make sure the selectedCar is defined
    if (!selectedCar) {
      console.log('Selected car is not available. Aborting booking.');
      return;
    }
  
    // Calculate the total price from the selectedCar
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Save the client_secret for later
        setClientSecret(response.data.client_secret);
        setShowCheckoutForm(true); // Show the payment section
      } else {
        // Handle failure
  ...
  {showCheckoutForm && (
    <Elements stripe={loadStripe('pk_test_51NMtCwAZgL2TxpbyteMi0Vbr3rcCTuFyx8fU7u8SidcNy0IRBebOzIvSgLlyYOBhWGSHUulY8xcBqXJlhbTqXlQQ009uT9bzCQ')}>
