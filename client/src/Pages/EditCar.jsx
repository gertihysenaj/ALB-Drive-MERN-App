...
function EditCar() {
    ...
    const handleSubmit = (event) => {
        ...
        // append only the selectedFile if it exists, else don't append the image at all
        if (selectedFile) {
            formData.append('image', selectedFile);
        }
    
        // append all other keys except 'img'
        ...
        axios.put(`http://localhost:8000/api/cars/${id}`, formData, { withCredentials: true })
            .then(res => {
                navigate(`/models`);
            })
            .catch(err => {
    ...
    return (
        <div className="createCar__container">
            <HeroPages name="Edit the Car" />
          <form className="createCar__form" onSubmit={handleSubmit}>
            <h2 className="createCar__title">Edit the Car</h2>
            {carData.img && <img className="createCar__preview-image" src={`http://localhost:8000/${carData.img.replace('\\', '/')}`} alt="Car Preview" />}
            <label>Name</label>
            ...
            <button className="createCar__btn-submit" type="submit">Update Car</button>
          </form>
        </div>
      );
    }
